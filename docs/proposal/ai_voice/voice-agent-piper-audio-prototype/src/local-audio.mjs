...
export function createLocalAudio({
  ...
  kokoroPython = process.env.PIPER_PYTHON || "python3",
  kokoroScript = process.env.PIPER_SCRIPT || "src/synthesize_piper.py",
  voice = process.env.PIPER_VOICE || "es_MX-claude-high",
  pitchScale = process.env.PIPER_PITCH_SCALE || "1.0",
  runCommand = run
} = {}) {
  return {
    ...
    async synthesize(text) {
      ...
      return inTemporaryDirectory(async (directory) => {
        ...
        if (audio.length < 44) throw new LocalAudioError("Piper no produjo audio válido.");
        return audio;
      });
    },
