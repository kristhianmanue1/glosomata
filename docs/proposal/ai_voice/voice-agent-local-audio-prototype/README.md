...
- Python 3.10 a 3.12 con `kokoro-mlx` (Apple Silicon/MLX).
- Descarga inicial de los pesos de Kokoro al primer uso.
- Navegador con `getUserMedia`, `AudioContext` y permiso de micrófono.

La voz predeterminada es `em_alex`, masculina y de español general; no es una
afirmación de acento mexicano. Su tono predeterminado se procesa localmente a
`0.90`, aproximadamente 10% más grave, conservando el ritmo. `em_santa` queda
